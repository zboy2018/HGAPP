...
<script type="text/babel">
  export default {
    ...
    methods: {
      ...
      getPanels() {
        return this.$children.filter(item => item.$options.name === 'scrolltabPanel');
      },
      bindEvent() {
      ...
      moveHandler(uid) {
        ...
        const panels = this.getPanels();
        console.log(panels.filter(item => item._uid == uid)[0]);
...
//        if (scrollBoxTop >= panelItemHeight * panelsLength - scrollBoxHeight-30 ) {
